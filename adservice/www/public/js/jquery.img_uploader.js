...
function setfileinfo(cnt_id, d, show_succ, customSettings)
{
    ...
    var img_url = d.url;
    $cnt.find('.uploaded-img-info').hide().html('<a href="' + d.url + '" title="点击在新窗口查看图片" target="uploaded_img_preview"><img src="' + img_url + '" height="16" width="16" style="margin:0;border:none"/>&nbsp;' + fn + '</a><div style="position:absolute;top:15px;border:1px solid #999;border-bottom:2px solid #666;border-right:2px solid #666;display:none;z-index:1000"><img src="' + img_url + '" style="margin:0;border:none;"/></div>').fadeIn()
    .mouseenter(function(){
...
$.fn.img_uploader = function(ext_options)
{
	var options = {
		...
		// Button Settings
		button_image_url : "/public/js/img_uploader/SmallSpyGlassWithTransperancy_17x18.png",
    ...
    if (ext_options.upload_url) {
        options.upload_url = ext_options.upload_url;
    }

    if (ext_options.file && ext_options.file.url) {
