...
class MY_Form_validation extends CI_Form_validation
{
    ...
    function valid_date($str)
    {
        ...
        $ret = strtotime($str);
        if ($ret < 0 || $ret === FALSE) {
            $this->set_message('valid_date', '%s不是有效的日期');

            return FALSE;
        }
        
        if (!preg_match('@^\d{4}-\d{2}-\d{2}$@', $str)) {
            $this->set_message('valid_date', '%s日期格式不正确，格式：yyyy-mm-dd');
    ...
    function safe_password($str)
    {
        ...
        if ($level < 3) {
            $this->set_message('safe_password',
                '您设置的%s太简单，必须包含数字、大小写字母、其它符号中的三种及以上'
            );
    ...
    function valid_mobile($str)
    {
        ...
        ($r = preg_match("/^(13|15|18)\d{9}$/",$str)) 
            || $this->set_message('valid_mobile', "%s({$str})不是合法的手机号");
        return !!$r;
    }
    ...
    function valid_phone($str)
    {
        ...
        ($r = preg_match("/^(((d{3}))|(d{3}-))?((0d{2,3})|0d{2,3}-)?[1-9]d{6,8}$/",$str)) 
            || $this->set_message('valid_phone', "%s({$str})不是合法的座机号");
        return !!$r;
    }
