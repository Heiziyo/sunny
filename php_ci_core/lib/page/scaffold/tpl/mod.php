...
<?php $scaffold_helper->beforeModFormRender(@$scaffold_item); ?>
    <?php
        ...
        <div class="controls">
            <?php
                ...
                if ($type == 'readonly') {
                    ...
                    $options = $field_config['options'];
                    $defaultOptions = array();
                    $item_val = isset($scaffold_item[$name]) ? $scaffold_item[$name] : NULL;
                    if ($item_val !== NULL) {
                        $defaultOptions = explode(',', $item_val);
                    }else if(isset($field_config['default'])){
                        $defaultOptions = explode(',', $field_config['default']);
                    }
                    foreach ($options as $op_val => $op_name) {
                        $id = "ck_{$name}_{$op_val}";
                        $defaultChecked = in_array($op_val, $defaultOptions) ? TRUE : FALSE;
                        echo '<label class="radio-inline">';
                        echo form_radio($name, $op_val, set_checkbox($name, $op_val, $defaultChecked));
                        echo $op_name;
                        echo '</label>';
                    }
    ...
    <?php if (empty($isAjax)) :?>
        <div class="form-actions">
            <?php $scaffold_helper->beforeActionRender(@$scaffold_item); ?>
            <?php if(!isset($scaffold_config['hideActionRender']) || $scaffold_config['hideActionRender'] == FALSE) : ?>
                <input type="submit" class="btn btn-primary" value="保存">
                <input type="button" onclick="history.go(-1)" class="btn ml20" value="取消">
            <?php endif ?>
            <?php $scaffold_helper->afterActionRender(@$scaffold_item); ?>
        </div>
    <?php endif;?>

<?php $scaffold_helper->afterModFormRender(@$scaffold_item); ?>
</form>
