...
class ScaffoldController extends Controller
{
    ...
    protected function _setScaffoldTplDir($dir)
    {
        $this->_scaffoldTplDir = $dir;
    }
    ...
    public function mlist()
    {
        ...
        $attrs = d(@$config['list']['attrs'], array());
        ...
        if ($GLOBALS['req_kw'] && isset($config['list']['keyword'])) {
            ...
            if (is_array($kwc) && isset($kwc['='])) {
                if (is_array($kwc['='])) {
                    foreach ($kwc['='] as $column) {
                        $subWhere[] = array($column => $kw);
                    }
                } else {
        ...
        $model = isset($config['list_model']) ? $config['list_model'] : $config['model'];
        $totalary=array();
        if(sizeof(@$config['totalfield'])>0){
            $totalitems = $model->select($where);
            foreach($totalitems as $k => $rows){
                foreach($config['totalfield'] as $fld ){
                    @$totalary[$fld]+=$rows[$fld];
                }
            }
            //var_dump($totalary);
        }
        ...
        //获取总数据
        if(!empty($attrs['select']) || !empty($attrs['group_by'])){
            $total = $this->selectCount($model, $where, $attrs);
        }else{
        ...
        $this->data['scaffold_item_total'] = $totalary;
    ...
    public function selectCount($m, $where, $attrs){

        $tmpAttrs = $attrs;
        if(isset($tmpAttrs['limit'])) unset($tmpAttrs['limit']);
        if(isset($tmpAttrs['offset'])) unset($tmpAttrs['offset']);

        $sql = Db_Sql::getInstance()->where($where, $tmpAttrs);

        if(isset($attrs['select'])) {
            $sql = 'SELECT ' . $attrs['select'] . ' FROM ' . $m->getTable() . $sql;
        }else{
            $sql = 'SELECT count(1) FROM ' . $m->getTable() . $sql;
        }
    ...
    public function delete()
    {
        ...
        foreach ($rows as $row) {
            if ($config['helper']->canDelete($row) === FALSE) {
                $this->_fail('不能被删除');
                return;
            }
    ...
    protected function _do($type, $config, $partial = NULL)
    {
        ...
        foreach ($config['fields'] as $fieldCfg) {
            ...
            if ($data[$name] === '') {
                if (isset($fieldCfg['type']) && $fieldCfg['type'] == 'date') {
                    $data[$name] = NULL;
                }
            }
        }
    ...
    {
        $scaffoldTplDir = d(
            $this->_scaffoldTplDir, 
            dirname(__FILE__) . '/page/scaffold/tpl'
        );

        $tplPath = $scaffoldTplDir . '/' . $tpl . '.php';
