...
abstract class Controller extends CI_Controller
{
    ...
    protected function _checkPermission($resource = NULL, $strict = TRUE)
    {
        ...
        if ( ! $resource) {
            ...
            if ($this->router->directory) {
                $resource = str_replace('/', '.', trim($this->router->directory, '/')) . '.' . $resource;
            }
        }
    ...
    {
        if(!empty($this->data)){
            $GLOBALS['current_username'] = $this->data['me']['username'];
            $GLOBALS['uid'] = $this->data['me']['id'];
            $GLOBALS['login_time'] = $this->data['me']['login_time'];
            $GLOBALS['login_ip'] = $this->data['me']['login_ip'];
        }
