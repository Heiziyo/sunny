 ...
 * @return array string
 */
function str4html($mixed, $type = ENT_QUOTES, $charset = 'utf-8') {
    if (is_string ( $mixed )) {
        return @htmlspecialchars ( $mixed, $type, $charset );
    } elseif (is_array ( $mixed )) {
        foreach ( $mixed as $key => $val ) {
            if (is_string ( $val )) {
                $mixed [$key] = @htmlspecialchars ( $val, $type, $charset );
            }
        }
        return $mixed;
    }
...
function getRequestParam($name, $default = "") {
    if (isset ( $_GET[$name] )) {
        return str4html ( $_GET[$name] );
    }
    return $default;
}

function postRequestParam($name, $default = "") {
    if (isset ( $_POST[$name] )) {
        return str4html ( $_POST[$name] );
    }
    return $default;
}
